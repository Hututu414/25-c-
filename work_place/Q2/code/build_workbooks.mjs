...
function numberFormat(header) {
  if (/^(N|K|Group|组别|孕妇数|n_|count|.*_N)$/i.test(header)) return "#,##0";
  if (/reliability|proportion|可靠性|比例/i.test(header)) return "0.0000";
  if (/R2|RMSE|MAE|variance|error|cost|objective|deviance|precision/i.test(header)) return "0.000000";
  if (/BMI|GA|timing|时点|孕周|sd|shift|cutpoint|q$/i.test(header)) return "0.0000";
  if (/pct|percent|improvement/i.test(header)) return "0.00";
  return null;
}
...
for (const specification of payload.workbooks) {
  ...
  for (const sheetSpec of specification.sheets) {
    ...
    for (let column = 0; column < columnCount; column += 1) {
      ...
      if (column > 0 || !/id|interval|scenario|week|周/i.test(sheetSpec.columns[column])) {
